...
ggplot(df_ts, aes(month)) + 
  geom_line(aes(y = (training-1.58), colour = "Actual")) + 
  geom_line(aes(y = (training_stl$time.series[,2]-1.58), colour = "Trend")) +
  xlab("Date") +
  ylab("Well ft") +
  labs(title = "Actual Well Feet vs. Trend", size = 15) +
  theme(legend.title = element_blank())

#Actual vs training seasonally adjusted

seas_pass=training-training_stl$time.series[,1]
plot(training, col = "grey", main = "Well Feet - Seasonally Adjusted", xlab = "", ylab = "Well Feet", lwd = 2)
lines(seas_pass, col = "red", lwd = 2)

trend = training_stl$time.series[,2]  + training_stl$time.series[,3]
plot(training, col = "grey", main = "Well Feet - Seasonally Adjusted", xlab = "", ylab = "Well Feet", lwd = 2)
